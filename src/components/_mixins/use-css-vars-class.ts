...
import { throwError } from "@/components/_utils";
import { c } from "@/components/_utils/cssr";

export function useVdThemeClass(
  ...
  if (!cssVarsRef) throwError("useVdThemeClass", "cssVarsRef is not passed");

  const mergedThemeHashRef = inject(
    VdConfigProviderInjectionKey,
    null
  )?.mergedThemeHashRef;
